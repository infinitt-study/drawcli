...
// 도형을 움직일 때 테두리 움직이면서 바뀌는 얘들(안보일정도로 빠른 속도로 생성)
...
// 교수님
// 옵저버 디자인 패턴의 한 방법
// mvc 의 옵저버 패턴을 mfc로 활용
// 변경 사실을 수신받는 곳
void CDrawView::OnUpdate(CView*, LPARAM lHint, CObject* pHint) //Long parmeter
{
	switch (lHint)
	{
...
void CDrawView::OnPrepareDC(CDC* pDC, CPrintInfo* pInfo)
{
	...
	pDC->SetMapMode(MM_ANISOTROPIC); //mapping 종류 중 하나 이건 원래 컴퓨터 상 0,0 인데 
	pDC->SetViewportExt(pDC->GetDeviceCaps(LOGPIXELSX), // 점하나 크게에 관한 비율 셋팅
	pDC->GetDeviceCaps(LOGPIXELSY)); //logical pixels y
	pDC->SetWindowExt(100, -100); // 100은 오른쪽, -100은 아래
	// 윈도우하고 1:! 맵핑 함
	...
	// 이게 오리지널 원점을 화면에 맞게 바꾸는 거임
	// 그래서 여기서 - 를 주는거임 그리드의 원점을 위해
	...
	if (!pDC->IsPrinting())
	{
		...
		if (dc.CreateCompatibleDC(pDC))
		{
			if (bitmap.CreateCompatibleBitmap(pDC, rect.Width(), rect.Height()))
			{
				...
				//OffsetViewportOrg(): 현재 뷰포트 원점의 좌표를 기준으로 뷰포트 원점의 좌표를 수정합니다.
...
void CDrawView::DrawGrid(CDC* pDC)
{
	...
	for (int x = rect.left / 100 * 100; x < rect.right; x += 100) // 얘는 가로 사이 넓이
	{
		if (x != 0)
		{
	...
	for (int y = rect.top / 100 * 100; y < rect.bottom; y += 100) // 얘는 세로 사이 넓이
	{
		if (y != 0)
		{
...
//// 얘를 주석 처리하면 엉뚱한 위치와 상하가 바뀜
// 디바이스 단위를 우리가 띄우는 뷰폼에 맞게 변환시켜주는 것.
void CDrawView::ClientToDoc(CPoint& point)
{
	CClientDC dc(this);
	////개체와 CClientDC 연결된 디바이스 컨텍스트가 창의 클라이언트 영역입니다.
	OnPrepareDC(&dc, NULL);// dc를 준
	dc.DPtoLP(&point);
	////DPtoLP(): 디바이스 단위를 논리 단위로 변환합니다.
}

// 만든 사각형의 포인트들 셀렉트할 때 선택할 수 있게 해줌
// 화면을 문서
void CDrawView::ClientToDoc(CRect& rect)
{
	...
	OnPrepareDC(&dc, NULL);
	dc.DPtoLP(rect);
	ASSERT(rect.left <= rect.right); //assert 유효성 검증: 맞으면 돌아가고 아니면 안돌아가는것
	ASSERT(rect.bottom <= rect.top); // 사각형은 늘 오른쪽 , 위쪽이 큼
}
...
//void CDrawView::ClientToDoc(CRect& rect)의 정반대
...
// 문서의 drag drop 이라 상관없음
int CDrawView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (CScrollView::OnCreate(lpCreateStruct) == -1) // 부모쪽에서 생성이 안되면 -1을 리턴
		return -1;

	//// register drop target
	if (m_dropTarget.Register(this))
		//Register(): 창을 유효한 삭제 대상으로 등록합니다.
		return 0;
	else
		return -1;
}
